...
const EncountersPage = () => {
    ...
    const blank = { patient_id: "", doctor_id: "", appointment_id: "", visit_date: "", follow_up_date: "", BP: "", chief_complaint: "", notes: "" };
    ...
    const filtered = encounters.filter(enc => {
        ...
        if (!q) return true;
        return [enc.id, patientName(enc.patient_id), doctorName(enc.doctor_id), enc.chief_complaint, fmtDate(enc.visit_date)]
                ...
                <DataTable
                    ...
                    rows={paginated.map(enc => (
                        <TR key={enc.id}>
                            ...
                            <TD muted>{enc.visit_date ? String(fmtDate(enc.visit_date)).split("T")[0] : "—"}</TD>
                            <TD>
                                <span className="px-2.5 py-1 rounded-full text-xs font-semibold border bg-teal-50 text-teal-700 border-teal-100">
                                    {enc.chief_complaint || "—"}
                                </span>
                            </TD>
                            <TD muted>{enc.follow_up_date ? String(fmtDate(enc.follow_up_date)).split("T")[0] : "—"}</TD>
                            <TD>
            ...
            {showModal && (
                <Modal title="Edit Encounter" onClose={closeModal} wide>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        {/* ✅ FIX: follow-up date cannot be before visit date */}
                        <Input
