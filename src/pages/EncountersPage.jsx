...
const EncountersPage = () => {
  ...
  const fetchEncounters = async () => {
    try {
      ...
      showToast("Failed to load encounters");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchPatients = async () => {
    try {
      ...
      setPatients(Array.isArray(data)
        ? data.map(p => ({
            value: p.id,
  ...
  const patientName = id =>
    patients.find(p => String(p.value) === String(id))?.label || "—";

  const doctorName = id =>
    doctors.find(d => String(d.value) === String(id))?.label || "—";
  ...
  const handleSave = async () => {
    ...
    try {
      // Use the new procedure API for both create and update
      const payload = {
        id: editingEncounter?.id || null,
        clinic_id: 1,
        ...
        created_by: "admin",
        // For now, empty arrays since we're not handling diagnoses/prescriptions in this modal
        diagnoses: [],
        prescriptions: []
      };

      const res = await fetch(`${API}/save_encounter_with_details`, {
        method: "POST",
  ...
  const handleDelete = async id => {
    ...
    try {
      ...
      if (!res.ok) {
        fetchEncounters();
        showToast(data?.error || "Failed to delete encounter");
      }

    } catch {
      fetchEncounters();
      showToast("Failed to delete encounter");
    }
  };

  const filtered = encounters.filter(enc =>
    [enc.id,
      patientName(enc.patient_id),
      doctorName(enc.doctor_id),
      enc.chief_complaint,
      enc.visit_date
  ...
  // 🔹 NEW API FUNCTION FOR PROCEDURE
  const saveEncounterWithDetails = async (data) => {
    try {
      ...
      console.log("Saving encounter with payload:", payload); // Debug log
      ...
      if (!result.encounter_id && !result.success) {
        throw new Error("Invalid response from server");
      }

      return result.encounter_id || null;

    } catch (err) {
      console.error("Error in saveEncounterWithDetails:", err);
      showToast(err.message || "Failed to save encounter");
      return null;
    }
  };
      ...
      {isLoading ? (
        ...
        <div>
          ...
          <DataTable
            ...
            search={search}
            onSearch={setSearch}
            searchPlaceholder="Search by patient, doctor, complaint…"
            columns={[
              "ID",
              "Patient",
              "Doctor",
            ...
            rows={paginated.map(enc => (
              ...
              <TR key={enc.id}>
                ...
                <TD muted>
                  {enc.visit_date
                    ? String(enc.visit_date).split("T")[0]
                    : "—"}
                </TD>
                ...
                <TD muted>
                  {enc.follow_up_date
                    ? String(enc.follow_up_date).split("T")[0]
                    : "—"}
                </TD>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <Input
                ...
                onChange={v => setField("chief_complaint", v)}
              />
