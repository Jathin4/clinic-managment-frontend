// pharmacySlip.jsx — Inventory Sale Receipt
// Usage (table row button): <pharmacySlip transaction={item} />
// Usage (programmatic after sale): triggerPrint(saleData)
//   saleData = { sale_invoice_number, patient_name, chief_complaint, items: [...salesList], date }
...
// ─── Normalise: accepts full-sale shape OR single OUT transaction row ──────────
const normalise = (raw) => {
  ...
  // Single OUT transaction row from table
  return {
    ...
    <tr>
      ...
      <td>
        ...
        ${item.dosage && item.dosage !== '—'
          ? `<div style="font-size:11px;color:#94a3b8;margin-top:2px">${item.dosage}</div>`
          : ''}
      </td>
      <td>${item.batch}</td>
      <td>
...
// ─── Named export — call programmatically after sale submission ───────────────
