      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {filteredClinics.map((c, i) => (
            <div 
              key={c.id} 
              className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm hover:shadow-md hover:border-teal-200 transition-all duration-300 opacity-0 animate-fade-in"
              style={{ animationDelay: `${i * 50}ms`, animationFillMode: 'forwards' }}
            >
              {/* Header with icon and title */}
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-teal-50 border border-teal-100 shadow-sm text-teal-700">
                    <Icons.Building />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-gray-900 text-sm leading-tight truncate">{c.name}</div>
                    <div className="flex items-center gap-1 mt-0.5">
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider bg-purple-50 text-purple-700 border border-purple-100 whitespace-nowrap">
                        {c.subscription_plan}
                      </span>
                    </div>
                  </div>
                ...
                <button 
                  ...
                  className="w-6 h-6 flex-shrink-0 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 hover:text-red-500 transition-all border border-transparent hover:border-gray-200 disabled:opacity-50">
                  <Icons.Trash />
                </button>
              </div>
              ...
              <div className="space-y-2 mb-3 pb-3 border-b border-gray-100">
                {c.address && (
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-gray-700 leading-tight min-w-0">
                      <div className="font-medium truncate">{c.city}, {c.state}</div>
                      <div className="text-xs text-gray-500 truncate">{c.pincode}</div>
                    </div>
                  </div>
                )}
                ...
                {c.phone && (
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-gray-700 truncate">{c.phone}</div>
                  </div>
                )}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <button className="p-1 rounded-lg hover:bg-blue-50 text-slate-400 hover:text-blue-600 transition-colors border border-transparent hover:border-blue-100">
                    <Icons.Edit />
                  </button>
                  <button className="p-1 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors border border-transparent hover:border-red-100">
                    <Icons.Trash />
                  </button>
                </div>
              </div>
