  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        <StatCard title="Total Patients"    value="1,842" change="+12%"  icon={Icons.Patient}      />
        <StatCard title="Today's Apts"      value="24"    change="+3"    icon={Icons.Calendar}    bgColor="#EDE9FE" />
        <StatCard title="Active Rx"         value="312"   change="+8%"   icon={Icons.Prescription} bgColor="#FEF3C7" />
        <StatCard title="Today's Revenue"   value="₹48,200" change="+5.2%" icon={Icons.DollarSign} bgColor="#DCFCE7" />
        <StatCard title="Monthly Revenue"   value="₹2.1L" change="+18%"  icon={Icons.TrendUp}     bgColor="#FCE7F3" />
        <StatCard title="Pending Payments"  value="₹34,500" change="-3 bills" icon={Icons.AlertTriangle} bgColor="#FEE2E2" positive={false} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="font-bold text-slate-800 mb-4 flex items-center gap-2">
            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>Today's Appointments
          </div>
          {MOCK_APPOINTMENTS.filter(a=>a.appointment_date==="2025-03-04").map(a=>(
            <div key={a.id} className="flex items-center gap-3 mb-3">
              <div className="w-8 h-8 rounded-full bg-teal-50 flex items-center justify-center text-teal-700 text-xs font-bold">{a.token_number}</div>
          ...
          {MOCK_PAYMENTS.map(pay=>{
            ...
            return (
              <div key={pay.id} className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm font-bold text-emerald-600">₹{pay.amount_paid.toLocaleString()}</span>
              </div>
            );
          })}
        </div>
